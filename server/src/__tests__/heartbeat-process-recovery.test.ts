...
describeEmbeddedPostgres("heartbeat orphaned process recovery", () => {
  ...
  it("does not reap a never-started run as active when both startedAt and processStartedAt are null (Greptile Issue 1 / CAN-3606)", async () => {
    // A run that has neither `startedAt` nor `processStartedAt` (e.g. an
    // in-memory adapter execution that never logged either anchor) must not
    // be silently re-classified as out-of-budget by the active ceiling. The
    // queue/lease ceiling above is the canonical gate for never-launched
    // runs; the active ceiling must skip such runs entirely.
    const { runId } = await seedRunFixture({
      adapterType: "opencode_local",
      agentStatus: "running",
      runStatus: "running",
    ...
    // Force the run into a never-started state. Both anchors are null and
    // createdAt is well past both ceilings — without the Greptile Issue 1
    // guard, the legacy `Number.POSITIVE_INFINITY` branch would have reaped
    // the run as active.
    await db
      .update(heartbeatRuns)
      .set({
    ...
    const reaperHeartbeat = heartbeatService(db);
    const result = await reaperHeartbeat.reapOrphanedRuns({
      // Disable the queue/lease ceiling so the test isolates the active
      // ceiling path. The active ceiling must still leave a never-started
      // run alone — it has no anchor to measure against.
      maxQueuedRunAgeMs: 0,
      maxActiveRunAgeMs: 60 * 60 * 1000,
    });
    expect(result).toEqual({ reaped: 0, runIds: [] });
